...
namespace GameOverlay.Drawing
{
	...
	public class Graphics : IDisposable
	{
        ...
        public GIFImage CreateImageGIF(string path)
        {
            if (!IsInitialized) throw new InvalidOperationException("The DirectX device is not initialized");

            return new GIFImage(_device, path);
        }
        ...
        public void DrawImageGIF(int currentFrameIndex, GIFImage image, float x, float y, float opacity = 1.0f)
        {
            if (!IsDrawing) throw ThrowHelper.UseBeginScene();
            FrameImageDisposal frameDisposal = image.frameImages[0].frameDisposal;
            float halfWidth, halfHeight;

            if (image.useGlobalBackgroundColor)
            {
                halfWidth = image.Width / 2;
                halfHeight = image.Height / 2;
                _device.DrawRectangle(new RawRectangleF(x - halfWidth, y - halfHeight, x + halfWidth, y + halfHeight), image.bgBrush.Brush);
            }

            switch(frameDisposal)
            {
                case FrameImageDisposal.Unspecified:
                    FrameImage currentFrame = image.frameImages[currentFrameIndex];
                    Rectangle frameRectangle = currentFrame.Rectangle;
                    halfWidth = currentFrame.Width / 2;
                    halfHeight = currentFrame.Height / 2;
                    frameRectangle.Left = frameRectangle.Left + x - halfWidth;
                    frameRectangle.Top = frameRectangle.Top + y - halfHeight;
                    frameRectangle.Right = frameRectangle.Right + x - halfWidth;
                    frameRectangle.Bottom = frameRectangle.Bottom + y - halfHeight;
                    _device.DrawBitmap(currentFrame.Frame, frameRectangle, opacity,
                        BitmapInterpolationMode.Linear);
                    break;
                case FrameImageDisposal.NotDispose:
                case FrameImageDisposal.RestoreBackground:
                    for(int i = 0; i <= currentFrameIndex; i++)
                    {
                        currentFrame = image.frameImages[i];
                        frameRectangle = currentFrame.Rectangle;
                        halfWidth = currentFrame.Width / 2;
                        halfHeight = currentFrame.Height / 2;
                        frameRectangle.Left = frameRectangle.Left + x - halfWidth;
                        frameRectangle.Top = frameRectangle.Top + y - halfHeight;
                    ...
                    break;
                case FrameImageDisposal.RestorePrevious:
		...
		/// <param name="opacity">A value indicating the opacity of the image. (alpha)</param>
		public void DrawImageGIF(int currentFrameIndex, GIFImage image, Point location, float opacity = 1.0f)
            => DrawImageGIF(currentFrameIndex, image, location.X, location.Y, opacity);
		...
		public void DrawText(Font font, float fontSize, IBrush brush, float x, float y, string text, ref BoxContainer boxContainer)
		{
			if (!IsDrawing) throw ThrowHelper.UseBeginScene();
            ...
            if(boxContainer.isCustomSize)
                textLayout = new TextLayout(_fontFactory, text, font.TextFormat, boxContainer.customWidth, boxContainer.customHeight);
            else
                textLayout = new TextLayout(_fontFactory, text, font.TextFormat, float.PositiveInfinity, float.PositiveInfinity);
            ...
            if(!boxContainer.isCustomSize)
            {
                boxContainer.customWidth = textLayout.Metrics.Width;
                boxContainer.customHeight = textLayout.Metrics.Height;
            }

            textLayout.WordWrapping = WordWrapping.Wrap;

            _device.DrawTextLayout(new RawVector2(x - boxContainer.customWidth / 2, y - boxContainer.customHeight / 2),
                textLayout, brush.Brush, DrawTextOptions.NoSnap);
		...
		/// <param name="text">The string to be drawn.</param>
		public void DrawText(Font font, float fontSize, IBrush brush, Point location, string text, ref BoxContainer boxContainer)
			=> DrawText(font, fontSize, brush, location.X, location.Y, text, ref boxContainer);
		...
		/// <param name="y">The y-coordinate of the starting position.</param>
		/// <param name="text">The string to be drawn.</param>
		public void DrawText(Font font, IBrush brush, float x, float y, string text, ref BoxContainer boxContainer)
			=> DrawText(font, font.FontSize, brush, x, y, text, ref boxContainer);
		...
		/// <param name="location">A Point structure including the starting position.</param>
		/// <param name="text">The string to be drawn.</param>
		public void DrawText(Font font, IBrush brush, Point location, string text, ref BoxContainer boxContainer)
			=> DrawText(font, font.FontSize, brush, location.X, location.Y, text, ref boxContainer);
